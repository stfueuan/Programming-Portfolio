...
bool FileVita::Write(void *buffer, const Int32 size)
{
	SceSSize return_value = sceIoWrite(file_descriptor_, buffer, size);

	return return_value < 0 ? false : true;
}

bool FileVita::Write(void *buffer, const Int32 size, const Int32 offset)
{
	SceSSize return_value = sceIoPwrite(file_descriptor_, buffer, size, offset);
