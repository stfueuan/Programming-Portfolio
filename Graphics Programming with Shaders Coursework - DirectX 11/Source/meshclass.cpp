...
bool MeshClass::Initialize(ID3D11Device* device, WCHAR* textureFilename, WCHAR* displacementMapName, WCHAR* bumpMapName) 
{
	bool result;
...
void MeshClass::Render(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* texture)
{
...
ID3D11ShaderResourceView* MeshClass::GetDisplacementMap()
{
	return m_displacementMap->GetTexture();
}

ID3D11ShaderResourceView* MeshClass::GetBumpMap()
{
	return m_bumpMap->GetTexture();
}

void MeshClass::GenerateMips(ID3D11DeviceContext* deviceContext)
{
...
bool MeshClass::InitializeBuffers(ID3D11Device* device)
{
	...
	vertices[0].texture = D3DXVECTOR2(0.0f, 2.0f);
	...
	vertices[2].texture = D3DXVECTOR2(2.0f, 0.0f);
	vertices[2].normal = D3DXVECTOR3(0.0f, 0.0f, -1.0f);

	vertices[3].position = D3DXVECTOR3(1.0f, -1.0f, 0.0f);  // Bottom right.
	vertices[3].texture = D3DXVECTOR2(2.0f, 2.0f);
...
void MeshClass::RenderBuffers(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* texture)
{
	unsigned int stride;
	unsigned int offset;
...
bool MeshClass::LoadTexture(ID3D11Device* device, WCHAR* filename, WCHAR* displacementMapName, WCHAR* bumpMapName)
{
	bool result;
