...
bool GlowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"texture.vs", L"glow.ps");
	if(!result)
	{
		return false;
	}
...
bool GlowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
				D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* textureAlpha, float glowType)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, textureAlpha, glowType);
	if(!result)
	{
		return false;
	}
...
bool GlowShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	//Pass integer to act as bool to determine whether we want to glow
	glowBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	glowBufferDesc.ByteWidth = sizeof(MatrixBufferType);
	...
	D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* textureAlpha, float glowType)
{
	...
	//give the pixel shader both the full colour and greyscale texture for glow effect
	ID3D11ShaderResourceView* pixelShaderTextures[2] = { texture, textureAlpha };

	// Set shader texture resource in the pixel shader.
	deviceContext->PSSetShaderResources(0, 2, pixelShaderTextures);

	// Send whether or not you want a greyscale image or not
	...
	dataPtr2 = (GlowTypeBuffer*)mappedResource.pData;

	dataPtr2->glowType = glowType;
