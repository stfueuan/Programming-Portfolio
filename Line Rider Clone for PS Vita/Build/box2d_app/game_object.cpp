...
float GameObject::Animate(int limit, int frames, float step_length, float ticks)
{
	frame_update += ticks;

	if (frame_update >= 0.1f) //change frames to a time value, not a number of frames value
	{
		frame_update = 0;
		frame_number++;
...
GameObject::state GameObject::get_state()
{
	//return GameObject::object_state;
}
